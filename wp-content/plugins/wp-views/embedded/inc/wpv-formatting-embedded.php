...
class WPV_Formatting_Embedded {
    ...
    static function init() {
		add_filter( 'the_content', array( 'WPV_Formatting_Embedded', 'resolve_wpv_noautop_shortcodes' ), 9 );
		add_filter( 'wpv_filter_wpv_the_content_suppressed', array( 'WPV_Formatting_Embedded', 'resolve_wpv_noautop_shortcodes' ), 9 );
		add_shortcode( 'wpv-noautop', array( 'WPV_Formatting_Embedded', 'wpv_shortcode_wpv_noautop' ) );
    }
	
	static function admin_init() {
		add_action( 'admin_print_footer_scripts', array( 'WPV_Formatting_Embedded', 'add_quicktags' ) );
	}
	...
	static function add_quicktags() {
		if ( wp_script_is( 'quicktags' ) ) {
			...
			<script type="text/javascript">
				QTags.addButton( 'wpv_noautop', 'raw', '[wpv-noautop]', '[/wpv-noautop]', 'r', '<?php echo esc_attr( __( 'Views raw output', 'wpv-views' ) ); ?>', 119 );
			</script>
			<?php
		}
	}
