...
function wpv_count_dissident_posts_from_template( $template_id, $content_type, $message_header = null ) {
    ...
    if ( $dissident_post_count > 0 ) {
        ...
        if ( $dissident_post_count > 1 ){
            ...
            $message = sprintf( __( '%d %s use a different Content Template.', 'wpv-views' ), $dissident_post_count , $type_label );
        } else {
            $type_label = $ptype->labels->singular_name;
            $message = sprintf(__('%d %s uses a different Content Template.', 'wpv-views'), $dissident_post_count, $type_label);
        }
...
*
* Checks whether the provided key corresponds to a Types custom field
*
* @patam $field_name string
*
* @return bool
*
* @since 1.9
...
function wpv_is_types_custom_field( $field_name ) {
