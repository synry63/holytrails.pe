            ...
            'display-options' => array(
                ...
                'fields' => array(
                    'show' => array(
                        ...
                        'default' => 'name',
						'documentation' => '<a href="http://codex.wordpress.org/Function_Reference/bloginfo">' . __( 'WordPress bloginfo function', 'wpv-views' ) . '</a>'
                    ),
                ),
            ),
        ),
            ...
            'display-options' => array(
                ...
                'fields' => array(
                    'info' => array(
						...
						'documentation' => '<a href="http://codex.wordpress.org/Function_Reference/get_userdata">' . __( 'WordPress get_userdata function', 'wpv-views' ) . '</a>'
                    ),
                ),
            ),
        ),
...
/*
  ...
  'post' means all wpv-post shortcodes but wpv-post-field in the Basic section plus non-Types fields in the Post field section
  'post-extended' means wpv-post-field and wpv-for-each shortcodes in the Basic section.
  
  'generic-extra' means some generic shortcodes under the Basic section.
  ...
  'view' means all available Views.
  'archives' means all WPAs - where is this being used?
...
function add_short_codes_to_js( $types, $editor ) {
	...
	foreach( $wpv_shortcodes as $shortcode ) {
		...
		if (
			...
			if ( $shortcode[0] == 'wpv-post-field' ) {
				// Output all the non-Types custom fields into a submenu.
	...
	if ( in_array( 'generic-extra', $types ) ) {
		// Add wpv-bloginfo
		$editor->add_insert_shortcode_menu(
			__('Site information', 'wpv-views'),
			'wpv-bloginfo',
			__('Basic', 'wpv-views'),
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-bloginfo', '" . __( 'Site information', 'wpv-views' ) . "', {}, '" . $nonce . "', this )"
		);
		// Add wpv-current-user
		$editor->add_insert_shortcode_menu(
			__('Current user info', 'wpv-views'),
			'wpv-current-user',
			__('Basic', 'wpv-views'),
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-current-user', '" . __( 'Current user info', 'wpv-views' ) . "', {}, '" . $nonce . "', this )"
		);
		$index += 1;
		// Add wpv-search-term
		$editor->add_insert_shortcode_menu(
			...
			'wpv-search-term',
			__('Basic', 'wpv-views'),
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-search-term', '" . __( 'Search term', 'wpv-views' ) . "', {}, '" . $nonce . "', this )"
		);
		$index += 1;
		// Add wpv-login-form
		$editor->add_insert_shortcode_menu(
			__('Login form', 'wpv-views'),
			'wpv-login-form',
			__('Basic', 'wpv-views'),
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-login-form', '" . __( 'Login form', 'wpv-views' ) . "', {}, '" . $nonce . "', this )"
		);
		$index += 1;
		// Add wpv-archive-link
		$editor->add_insert_shortcode_menu(
			__('Post archive link', 'wpv-views'),
			'wpv-archive-link',
			__('Basic', 'wpv-views'),
			"WPViews.shortcodes_gui.wpv_insert_popup('wpv-archive-link', '" . __( 'Post archive link', 'wpv-views' ) . "', {}, '" . $nonce . "', this )"
		);
		$index += 1;
	}
	
	// If WPML is enabled and String translation is active, add a Translatable string shortcode to the Basic submenu
	// This only works with dialogs related to posts - the ones for Views listing terms or users get this on their own
	if (
		isset( $sitepress )
	...
	*/
	...
	if ( in_array( 'body-view-templates', $types ) ) {
        // ct-editor-deprecated: this condition is no longer necessary
		global $pagenow, $typenow;
		if (
			"view-template" != $typenow
			|| ! in_array( $pagenow, array( 'post-new.php' ) )
		) {
			// Do not add this section on the ad-new-ct admin page!!
			$values_to_prepare = array();
			$values_to_prepare[] = 'view-template';
			...
			if ( $WP_Views->is_archive_view( $view->ID ) ) {
				...
				if ( in_array( 'view', $types ) ) {
					...
					if ( $WP_Views->does_view_have_form_controls( $view->ID ) ) {
						...
						$editor->add_insert_shortcode_menu(
							$view->post_title,
							'wpv-form-view name="' . esc_html( $view->post_title ) . '"',
							__( 'View', 'wpv-views' ),
							'WPViews.shortcodes_gui.wpv_insert_view_form_popup(\'' . $view->ID . '\', \'' . esc_js( $view->post_title ) . '\', \'' . $current_id . '\', \'' . $nonce . '\')'
						);
					} else {
						$editor->add_insert_shortcode_menu(
				...
				if ( in_array( 'post-view', $types ) ) {
					...
					// Add differently for parametric search ones
					if ( $view_type == 'posts' ) {
						if ( $WP_Views->does_view_have_form_controls( $view->ID ) ) {
							global $pagenow;
							$current_id = 0;
	...
	/*
    $editor->add_insert_shortcode_menu(
		$wpv_shortcodes['wpv-archive-title'][1],
		$wpv_shortcodes['wpv-archive-title'][0],
		__( 'Basic', 'wpv-views' )
	);
    $index += 1;
	*/
