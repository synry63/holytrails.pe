...
var wpv_ct_editor_l10n = wpv_ct_editor_l10n || {};
var wpv_ct_editor_ct_data = wpv_ct_editor_ct_data || {};
var ajaxurl = ajaxurl || '';
...
WPViews.CTEditScreen = function( $ ) {
     ...
     * @type string
     */
    self.update_nonce = $('#js-wpv-ct-update-nonce').val();
    ...
    self.ViewModel = function(ct_data, section_data) {
        ...
        vm.processUpdateResults = function(propertiesToCheck, unsavedMessage, savedMessage, messageContainer, updateResults) {
            ...
            var haveRelevantUpdates = (_.intersection(updateResults.propertiesUpdated, propertiesToCheck).length > 0);
            ...
            if(haveRelevantUpdates) {
                ...
                if(haveFailedUpdates) {
                    ...
                    var errorMessage = (updateWithErrorMessage == undefined) ? unsavedMessage : updateWithErrorMessage.message;

                    self.showErrorMessage(messageContainer, errorMessage);
        ...
        /* These two properties are an example of how we could implement an in-place
         * input validation. */
        vm.titleIsLastInputAccepted = ko.observable(true);

        vm.title = ko.pureComputed({
            read: vm.titleAccepted,
            write: function(newTitle) {
                // Validation could happen here
                vm.titleAccepted(newTitle);
                vm.titleIsLastInputAccepted(true);
            }
        });
         ...
         * @since 1.9
         */
        vm.isTitleSectionUpdateNeeded = ko.pureComputed(_.partial(vm.isSectionUpdateNeeded, vm.titleSectionProperties, vm.isTitleSectionUpdating));
        ...
        vm.lastUpdateResults.subscribe(_.partial(
            ...
            vm.titleSectionProperties,
            self.l10n.title_section.unsaved,
            self.l10n.title_section.saved,
            self.titleSectionMessageContainer));
        ...
        /**
         * Show/hide button text for CSS editor.
         *
         * @since 1.9
         */
        vm.templateCssToggleButtonText = ko.pureComputed(function() {
            //noinspection JSUnresolvedVariable
            return (vm.isCssEditorExpanded() ? self.l10n.content_section.button_css.close : self.l10n.content_section.button_css.open);
        });
        ...
        vm.templateJsToggleButtonText = ko.pureComputed(function() {
            //noinspection JSUnresolvedVariable
            return (vm.isJsEditorExpanded() ? self.l10n.content_section.button_js.close : self.l10n.content_section.button_js.open);
        });
        ...
        vm.isSaveAllButtonEnabled = ko.pureComputed(function() {
            return (vm.changedProperties().length > 0 && vm.updatingProperties().length == 0);
        });
        ...
        vm.lastUpdateResults.subscribe(function(updateResults) {
            ...
            if(haveFailedUpdates) {
                //noinspection JSUnresolvedVariable
                self.showErrorMessage(messageContainerSelector, self.l10n.editor.unsaved);
            } else {
                self.showSuccessMessage(messageContainerSelector, self.l10n.editor.saved);
                self.highlight_action_bar();
            }
        });
        ...
        // Apply knockout bindings (now the magic happens).
        ko.applyBindings(vm);
    ...
    /**
     * Collection of previous messages that should be removed when a new one is being
     * displayed in the same place.
     *
     * Contains object with two properties:
     * - selector: CSS selector of the message container
     * - messageObject: the wpvToolsetMessage object used to hide the message.
     *
     * @type [{selector: {string}, messageObject: {*}}]
     *
     * @since 1.9
     */
    ...
    self.showMessage = function(type, stay, fadeOut, selector, text) {
        ...
        var messagesToDelete = _.where(self.previousMessages, {selector: selector});
     ...
     *
     * Store the message object in self.previousMessages so that it will be removed next time another message shows
     * in the same place.
     *
     * @param selector {string} CSS selector for the message container.
     * @param text {string} Text of the message.
     ...
     * @since 1.9
     */
    self.showErrorMessage = function(selector, text) {
        var message = {
            selector: selector,
     ...
     * @since 1.9
     */
    self.highlight_action_bar = function() {
    ...
    /*$(document).on('click', '.js-wpv-trash', function() {
     // postponed to Views 1.10
     });*/
    ...
    self.init = function() {
        ...
        // Ensure Toolset compatibility (CRED button)
