...
function wpv_update_title_description_callback() {
	...
	if ( ! empty( $existence_check ) ) {
		$message = '';
		if ( $existence_check->post_name == $_POST["slug"] ) {
			if ( $existence_check->post_title == $_POST["title"] ) {
				$message = __( 'Another item with that name and slug already exists. Please use other values.', 'wpv-views' );
			} else {
				$message = __( 'Another item with that slug already exists. Please use another slug.', 'wpv-views' );
			}
		} else {
...
function wpv_duplicate_this_view_callback() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'wpv_duplicate_view_nonce' ) ) {
		die( "Security check" );
	}
    ...
    $original_view = WPV_View::create( $post_id );
    if( null == $original_view ) {
        die( 'error' );
    }

    // Duplicate it and print it's ID.
...
work in progress for views-132

add_action('wp_ajax_wpv_create_wpa_for_loop_popup', 'wpv_create_wpa_for_loop_popup_callback');

function wpv_create_wpa_for_loop_popup_callback() {
    wpv_ajax_authenticate( 'wpv_wp_archive_arrange_usage' );

    $selected_loop_slug = wpv_getpost( 'loop_slug', null );
    $selected_loop_info = null;

    global $WPV_view_archive_loop;
    $loops = $WPV_view_archive_loop->get_archive_loops( 'all' );
    foreach( $loops as $loop_info ) {
        if( $loop_info['slug'] == $selected_loop_slug ) {
            $selected_loop_info = $loop_info;
            break;
        }
    }

    if( null == $selected_loop_info ) {
        wp_send_json_error( 'Invalid loop slug.' );
    }

    ?>
    <div class="wpv-dialog js-wpv-dialog-create-wpa-for-loop">
        <div class="wpv-dialog-header">
            <?php
            printf(
                '<h2>%s <strong>%s</strong></h2>',
                __( 'Name of WordPress Archive for', 'wpv-views' ),
                sanitize_text_field( $selected_loop_info['display_name'] )
            );
            ?>
            ...
            <div class="wpv-dialog-content">
                <p>
                    <?php
                    printf(
                        '<input type="text" class="js-wpv-archive-name" placeholder="%s" value="%s"/>',
                        esc_attr( __( 'WordPress Archive name', 'wpv-views' ) ),
                        $selected_loop_info['display_name']
                    );
                    ?>
                </p>
                <input type="hidden" class="js-wpv-loop-slug" value="<?php echo esc_attr( $selected_loop_slug ); ?>" />
                <div class="js-error-container"></div>
            </div>
            <div class="wpv-dialog-footer">
                <button class="button-secondary js-dialog-close" type="button">
                    <?php _e('Cancel', 'wpv-views'); ?>
                </button>
                <button class="button-secondary js-wpv-submit" disabled="disabled"
                        <?php
                            //data-error="<?php echo esc_attr(__('A WordPress Archive with that name already exists. Please use another name.', 'wpv-views')); ?>"
                            // data-url="<?php echo admin_url('admin.php?page=view-archives-editor&amp;view_id=');?>"
                        ?>
                    >
    ...
    $loops = $WPV_view_archive_loop->_get_post_type_loops();
		...
		<form id="wpv-add-wp-archive-for-loop-form">
            <div class="wpv-dialog-content">
				...
				<p>
					<input type="text" value="<?php echo $_POST['for_whom']; ?>" class="js-wpv-new-archive-name wpv-new-archive-name" placeholder="<?php echo esc_attr( __('WordPress Archive name','wpv-views') ) ?>" name="wpv-new-archive-name">
				</p>
 ...
 *
 * TODO check where this is still used... maybe deprecate?
 *
 * @since 1.3.0
 */
 ...
 * @param string $nonce_parameter Name of the POST parameter containing nonce value. Defaults to "wpnonce".
 * @param string $capability_needed Capability that user has to have in order to pass the check.
 *     Default is "manage_options".
 * @param bool $die_only If true, the error message will be echoed as a simple string instead of using wp_json_error().
 *     Default is false.
 *
 * @since 1.9
 */
function wpv_ajax_authenticate( $nonce_name, $nonce_parameter = 'wpnonce', $capability_needed = 'manage_options', $die_only = false ) {
    $error_message = null;

    if ( ! current_user_can( $capability_needed ) ) {
        $error_message = 'Untrusted user';
    }

    if ( ! wp_verify_nonce( wpv_getpost( $nonce_parameter, '' ), $nonce_name ) ) {
