...
//add_filter('the_content', 'wpv_resolve_wpv_if_shortcodes', 8);
	...
	while ($counts) {
		foreach($matches[0] as $match) {
			...
			if ( 0 !== preg_match( "/$pattern/s", $match, $match_data ) ) {
				...
				// Also, it prevents us from accidentally transforming lower-than into whatever on the content itself.
				$match_corrected = str_replace( $match_data[5], 'wpv-b64-' . base64_encode( $match_data[5] ), $match_corrected );
				
				$match_attributes = shortcode_parse_atts( $match_data[3] );
...
//add_filter('wpv-pre-do-shortcode', 'wpv_parse_wpv_if_shortcodes', 4);
