		...
		*/
		$requires_parent_term = apply_filters( 'wpv_filter_requires_parent_term', $requires_parent_term, $view_settings );
			...
			if ( $this->get_parent_view_taxonomy() ) {
				$attr_term = $this->get_parent_view_taxonomy();
			}
		}
		$attr_user = '';
		$requires_parent_user = false;
		...
		*/
		...
		if ( $requires_parent_user ) {
			if ( $this->get_parent_view_user() ) {
				$attr_user = $this->get_parent_view_user();
			}
		}

		return $this->current_view . '-' . md5( serialize( $attr ) ) . md5( $attr_post ) . md5( $attr_term ) . md5( $attr_user );
	...
	*/
		...
		if ( $post->post_type != 'view-template' ) {
			...
			add_short_codes_to_js( 
				array( 
					...
					'generic-extra', // wpv-post shortcodes, extended (maybe other generic shortcodes too)
	...
	function get_meta_keys( $include_hidden = false ) {
		...
		if ( $cf_keys == null ) {
			...
			$cf_keys_limit = apply_filters( 'wpv_filter_wpv_get_postmeta_keys_limit', $cf_keys_limit );
...
';
	...
	*/
	...
	function wpv_admin_enqueue_scripts( $hook ) {
		...
		wp_register_script( 'views-shortcodes-gui-script' , WPV_URL_EMBEDDED . '/res/js/views_shortcodes_gui.js', array('jquery','jquery-ui-dialog', 'suggest', 'views-utils-script', 'jquery-ui-tabs'), WPV_VERSION);
		...
		wp_register_style(
				...
				array( 'toolset-font-awesome', 'toolset-colorbox', 'views-notifications-css', 'views-admin-dialogs-css', 'select2' ),
				WPV_VERSION );
...
function wpv_views_plugin_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
	...
	if ( $plugin_file == $this_plugin ) {
		$plugin_meta[] = sprintf(
				'<a href="%s">%s</a>',
				'https://wp-types.com/version/views-1-9-1/?utm_source=viewsplugin&utm_campaign=views&utm_medium=release-notes-admin-notice&utm_term=Views 1.9.1 release notes',
				__( 'Views 1.9.1 release notes', 'wpv-views' ) 
			);
	}
	return $plugin_meta;
}
