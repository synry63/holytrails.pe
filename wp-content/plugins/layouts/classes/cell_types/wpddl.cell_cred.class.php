...
if( !function_exists('register_cred_cell_init') )
{
	function register_cred_cell_init() {
		if ( function_exists('register_dd_layout_cell_type') ) {
			register_dd_layout_cell_type ( 'cred-cell',
				array (
					'name'						=> __('CRED form', 'ddl-layouts'),
					'description'				=> __('Display a CRED form which allows users to create and edit Posts, Pages and Custom content from the front-end.', 'ddl-layouts'),
					'category'					=> __('Forms', 'ddl-layouts'),
					'cell-image-url'					=> DDL_ICONS_SVG_REL_PATH.'cred-form.svg',
					'button-text'				=> __('Assign CRED form cell', 'ddl-layouts'),
					'dialog-title-create'		=> __('Create a new CRED form cell', 'ddl-layouts'),
					'dialog-title-edit'			=> __('Edit CRED form cell', 'ddl-layouts'),
					'dialog-template-callback'	=> 'cred_cell_dialog_template_callback',
					'cell-content-callback'		=> 'cred_cell_content_callback',
					'cell-template-callback'	=> 'cred_cell_template_callback',
					'cell-class'				=> '',
                    'has_settings' => false,
	...
	add_action( 'init', 'register_cred_cell_init', 12 );
					...
					<fieldset>
						<div class="fields-group">
							<label class="radio">
								...
								<?php _e('Create a new Form', 'ddl-layouts');?>
							</label>
							<span class="desc ddl-form-indent js-ddl-newcred"><?php _e('CRED forms allow you to create content or edit your content. Choose what you want this form to do and the type of content it will work with.', 'ddl-layouts'); ?></span>
				...
				<fieldset class="js-ddl-select-existing-cred">
					...
					<?php
						...
						foreach ( $posts as $post ) :
							...
							echo ddl_cred_get_option_element($post->ID,
															 $post->post_title,
...
-			<p><strong><?php _e('This form is used to %EDIT% %POST_TYPE%', 'ddl-layouts'); ?></strong></p>
		...
		<div id="ddl-cred-preview-cred-not-found" style="display:none">
			<h2><?php _e('The CRED form was not found. It may have been deleted.', 'ddl-layouts'); ?></h2>
		</div>
	...
	add_action('wp_ajax_ddl_create_cred_form', 'ddl_create_cred_form');
	function ddl_create_cred_form(){
		...
		if (defined( 'CRED_CLASSES_PATH' )) {
			...
			$result['form_id'] = $id;
			if ($id) {
				$result['option'] = ddl_cred_get_option_element($id,
																$_POST['name'],
																$_POST['mode'],
																$_POST['post_type']
																);
				...
				$result['error'] = __('Could not create the CRED form', 'ddl-layouts');
			}
	    }		
	...
	add_action('wp_ajax_ddl_delete_cred_forms', 'ddl_delete_cred_forms');
	...
	add_action('wp_ajax_ddl_get_option_for_cred_form', 'ddl_get_option_for_cred_form_callback');
	function ddl_get_option_for_cred_form_callback () {
		...
		$result['option'] = ddl_cred_get_option_element($_POST['cred_id'],
											 $post_title,
											 $form->fields['form_settings']->form['type'],
											 $form->fields['form_settings']->post['post_type']);
