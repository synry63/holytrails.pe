...
class WPDD_layout_cell_post_content_views_factory extends WPDD_layout_cell_factory{
						...
						<ul>
							...
							<?php if ($views_1_6_available): ?>
								<li class="js-post-content-ct js-ct-selector" style="text-align: right;">
                                    ...
                                    <a href="#" class="js-create-new-ct button button-large"><?php _e('Create a new one', 'ddl-layout'); ?></a>
								</li>
							<?php endif; ?>
						</ul>
					</div>
	...
	private function show_posts_dropdown($post_type, $name, $selected = 0, $page = 1, $paged = 1) {
		...
		if ( ! empty($pages) ) {
            ...
            if( $total > $count * $page )
                $output .= "\t<option value=\"-2\" class=\"js-show-more-posts-options show-more-posts-options\">". __('Show more', 'dd-layouts') ."</option>";
			$output .= "</select>\n";
		}
