	...
	*/
	...
	function release_notes( $notices ) {
		if ( current_user_can( 'activate_plugins' ) ) {
			...
			if ( isset( $dismissed_notices['wpv_release_notes_onenineone'] ) ) {
				return $notices;
			} else {
				$notice_text = '<p>'
					...
					. '" class="button button-primary button-primary-toolset">'
					. __( 'Views 1.9 release notes', 'wpv-views' )
					. '</a>';
				
				global $wp_version;
				if ( version_compare( $wp_version, '4.2.2', '<' ) ) {
				$notice_text .= '  <a class="button button-secondary js-wpv-dismiss" href="' . esc_url( add_query_arg( array( 'wpv_dismiss_global_notice' => 'wpv_release_notes_onenineone' ) ) ) . '">'
					. __( 'Dismiss', 'wpv-views' )
					. '</a>';
				}
				...
				$notices['wpv_release_notes_onenineone'] = $args;
			}
		}
		return $notices;
	}
