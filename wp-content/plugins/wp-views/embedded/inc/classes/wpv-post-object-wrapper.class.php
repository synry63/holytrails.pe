...
abstract class WPV_Post_Object_Wrapper {
     ...
     */
    public static function is_name_used_base( $name, $post_type, $except_id = 0 ) {
        global $wpdb;
     ...
     *          indexed by names of properties that caused them.
    ...
    public function update_transaction( $data ) {
        ...
        foreach( $data as $property_name => $value ) {
            ...
            if( method_exists( $this, $validate_method_name ) )  {
                ...
                try {
                    ...
                    // Validation has failed with a message. We can stop trying now.
                    $can_update_everything = false;
                    $error_messages[ $property_name ] = $e->getUserMessage();
                    break;
        ...
        if( !$can_update_everything ) {
            return array( 'success' => false, 'partial' => false, 'error_messages' => $error_messages );
        }
