...
class WPV_Content_Template_Embedded extends WPV_Post_Object_Wrapper {
    ...
    /**
     * Serialized array of Types fields that are being used by this CT.
     *
     * Used by Module Manager.
     *
     * @todo verify this description
     */
    const POSTMETA_TEMPLATE_FIELDS = '_wpv_view_template_fields';
    ...
    public static function is_name_used( $name, $except_id = 0 ) {
        return WPV_Post_Object_Wrapper::is_name_used_base( $name, WPV_Content_Template_Embedded::POST_TYPE, $except_id );
    }
    ...
    static function get_template_by_name( $post_name ) {
        return WPV_Content_Template_Embedded::create( WPV_Content_Template_Embedded::get_template_id_by_name( $post_name ) );
    }
    ...
    /**
     * @return string Output mode: 'WP_mode' for automatically inserted paragraphs or 'raw_mode' for manual paragraphs.
     * @since 1.9
     */
    protected function _get_output_mode() {
        $output_mode = $this->get_postmeta( WPV_Content_Template_Embedded::POSTMETA_OUTPUT_MODE );
        if( !in_array( $output_mode, array( 'WP_mode', 'raw_mode' ) ) ) {
            $defaults = $this->get_postmeta_defaults();
            return $defaults['_wpv_view_template_mode'];
        }
        return $output_mode;
    }
