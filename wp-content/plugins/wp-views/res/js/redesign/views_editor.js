...
WPViews.ViewEditScreen = function( $ ) {
	...
	*/
	...
	self.init_dialogs = function() {
		// Pagination events dialog
		$( "#js-wpv-dialog-views-frontend-events" ).dialog({
			autoOpen: false,
			modal: true,
			minWidth: 450,
			show: { 
	...
	self.insert_frontend_event_handler = function() {
		...
		$( '#js-wpv-dialog-views-frontend-events .js-wpv-frontend-event-gui:checked' ).each( function() {
			thiz_insert = "jQuery( document ).on( '" + $( this ).data( 'event' ) + "', function( event, data ) {";
			thiz_insert += "\n\t\n";
			thiz_insert += "});";
			thiz_insert_array.push( thiz_insert );
			$( this ).prop( 'checked', false );
		});
		$( '#js-wpv-dialog-views-frontend-events' ).dialog( "close" );
	...
	$( document ).on( 'click', '.js-wpv-views-frontend-events-popup', function() {
		...
		dialog = $( '#js-wpv-dialog-views-frontend-events' ).dialog('open').dialog({
            title: wpv_views_editor_texts.frontend_events_dialog_title,
            width: 770,
