...
*
*/
// ct-editor-deprecated because our post types have no longer a show_ui = true, there should be no admin listings.
...
function wpv_layout_taxonomy_V( $menu ) { // NOT DEPRECATED at all: used to generate the V icon popup for taxonomy Views.
    ...
    if ( 
		...
		$menu[$basic][$translatable_string_title] = array( 
			$translatable_string_title, 
			'wpml-string', 
			$basic, 
...
function wpv_layout_users_V( $menu ) {
    ...
    $user_shortcodes = array(
		__('User ID', 'wpv-views') => 'wpv-user field="ID"',
        __('User Email', 'wpv-views') => 'wpv-user field="user_email"',
        __('User Login', 'wpv-views') => 'wpv-user field="user_login"',
        __('First Name', 'wpv-views') => 'wpv-user field="user_firstname"',
        __('Last Name', 'wpv-views') => 'wpv-user field="user_lastname"',
        __('Nickname', 'wpv-views') => 'wpv-user field="nickname"',
        __('Display Name', 'wpv-views') => 'wpv-user field="display_name"',
        __('Description', 'wpv-views') => 'wpv-user field="description"',
        __('Yahoo IM', 'wpv-views') => 'wpv-user field="yim"',
        __('Jabber', 'wpv-views') => 'wpv-user field="jabber"',
        __('AIM', 'wpv-views') => 'wpv-user field="aim"',
        __('User Url', 'wpv-views') => 'wpv-user field="user_url"',
        __('Registration Date', 'wpv-views') => 'wpv-user field="user_registered"',
        __('User Status', 'wpv-views') => 'wpv-user field="user_status"',
        __('User Spam Status', 'wpv-views') => 'wpv-user field="spam"',
	);
    foreach ( $user_shortcodes as $shortcode_title => $shortcode_output ) {
		$menu[$basic][$shortcode_title] = array( 
			$shortcode_title, 
			$shortcode_output, 
			$basic, 
			''
...
function wpv_create_content_template( $title, $suffix = '', $force = true, $content = '' ) {
    ...
    if( $force ) {
        $ct = WPV_Content_Template::create_new( $template_title, true );
    } else {
        ...
        $ct = WPV_Content_Template::create_new( $template_title, false );
    }
...
// todo why is wpv_generate_view_loop_output not used instead??
// todo handle undefined, invalid or unexpected (1, -1) values
function wpv_create_bootstrap_meta_html ($cols, $ct_title, $meta_html) {
    ...
    // todo wpv_settings should be doing that
