...
final class WPV_Content_Template extends WPV_Content_Template_Embedded {
     ...
     */
    public static function create( $ct ) {
        try {
     ...
     * @since 1.9
     */
    public static function create_new( $title, $adjust_duplicate_title = true ) {
        ...
        $ct = WPV_Content_Template::create( $post_id );
    ...
    /**
     * Update information about custom fields used in the CT.
     *
     * The POSTMETA_TEMPLATE_FIELDS is being used by Module Manager at the moment.
     *
     * @param string|null $new_content Post content to check or null if it should be taken from the post_content property.
     *
     * @since 1.9
     */
    public function update_field_values( $new_content = null ) {

        $post_content = ( null != $new_content ) ? $new_content : $this->content;

        // search for shortcodes
        $match_count = preg_match_all( "/\\[(wpv-|types).*?\\]/i", $post_content, $matches );

        // iterate 0-level shortcode elements
        if ( $match_count > 0 ) {
            $template_fields = serialize( $matches[0] );
            $this->update_postmeta( WPV_Content_Template_Embedded::POSTMETA_TEMPLATE_FIELDS, $template_fields );
        }
    ...
    public function clone_this( $title, $adjust_duplicate_title = true ) {
        ...
        // Create new CT
        $cloned_ct = WPV_Content_Template::create_new( $title, $adjust_duplicate_title );
        if( null == $cloned_ct ) {
            return null;
        }
    ...
    protected function _validate_title( $value ) {
        ...
        if( WPV_Content_Template_Embedded::is_name_used( $sanitized_value, $this->id ) ) {
            throw new WPV_RuntimeExceptionWithMessage(
                '_validate_title failed: name is already being used for another CT',
                __( 'Another item with that slug or title already exists. Please use another title.', 'wpv-views' ) );
        }
    ...
    protected function _validate_slug( $value ) {
        ...
        if( WPV_Content_Template_Embedded::is_name_used( $sanitized_value, $this->id ) ) {
            throw new WPV_RuntimeExceptionWithMessage(
                ...
                __( 'Another item with that slug or title already exists. Please use another slug.', 'wpv-views' )
            );
        }
    ...
    protected function _set_content_raw( $post_content ) {
        ...
        if ( $original_post_content != $post_content ) {
            ...
            // Post-update actions
            $this->update_field_values( $post_content );
