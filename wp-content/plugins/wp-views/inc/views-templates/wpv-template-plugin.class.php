...
class WPV_template_plugin extends WPV_template
{
	...
	*/
    ...
    function initialize_if_wpml_copy() {
        ...
        // not a new CT; just get translations and extract original post ID
    ...
    function view_settings_meta_box() {
		...
		if ( ! empty( $layout_loop_template_for_view_id ) ) {
			...
			$view_loop_link = esc_url( add_query_arg(
				array( 'page' => 'views-editor', 'view_id' => $layout_loop_template_for_view_id ),
				admin_url( 'admin.php' ) 
			) );
			if ( $WP_Views->is_archive_view( $layout_loop_template_for_view_id ) ) {
				$view_loop_type_text = __( 'WordPress Archive', 'wpv-views' );
				$view_loop_link = esc_url( add_query_arg(
					array( 'page' => 'view-archives-editor', 'view_id' => $layout_loop_template_for_view_id ),
					admin_url( 'admin.php' ) 
				) );
			}
			$layout_loop_template_notice = '';
			if ( $view_loop_status == 'publish' ) {
				$layout_loop_template_notice = sprintf( 
	...
	*/
    // ct-editor-deprecated
    // @todo explore for scripts for CRED button
    ...
    function save_post_actions( $pidd, $post ) {
        ...
        if ( 
            ...
            // generate unique slug if it's empty
    ...
    // todo re-implement on new CT edit page
    // ct-editor-deprecated
 ...
 *
 * @deprecated Consider using WPV_Content_Template::update_field_values() directly.
 *
 * @param int $pidd post ID
...
function wpv_view_template_update_field_values( $pidd, $post = null ) {
    ...
    $ct = WPV_Content_Template::create( ( null != $post ) ? $post : $pidd );
    if( null != $ct ) {
        $ct->update_field_values();
    }
